...
@st.cache_data
def load_data():
    file_path = "sorce.xlsx"  # Change if needed
...
page = st.sidebar.radio("Go to", ["📈 Dashboard", "📉 Trend Analysis & Clustering"])
...
if df is not None:
    ...
    if page == "📈 Dashboard":
        ...
        # **Bar Chart - Total Trade Value**
        with col1:
            st.subheader(f"Total Trade Value - {selected_company} ({selected_year})")
            chart_data = yearly_trade_value[selected_company].reset_index()
            st.bar_chart(chart_data, x="Year", y=selected_company, use_container_width=True)
    ...
    elif page == "📉 Trend Analysis & Clustering":
        st.title("📊 Stock Trend Analysis & Clustering")
